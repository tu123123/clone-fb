...
const NotiItem=({value}:{value:any})=>{
    return <div className="NotiItem">
    <Avatar img={value.userSend.imgURL}></Avatar> <div className="NotiItem-content">
        {value.userSend.name}<br></br>
        <span style={{
            color:'white'
        }}>{value.message}</span></div>
    </div>
}
...
export default  function Header(){
    ...
    useEffect(()=>{
        window.addEventListener('click',(e:any)=>{
            ...
            if(e.target.className=='user-button'||e.target.className=='opennoti')
                {
                    setOpenNoti(true)
                }
            if((e.target.className!='user-button'&&e.target.className!='opennoti')&&e.target.className!='NotiItem'&&e.target.className!='NotiItem-content'&&e.target.className!='notification')
                {
                    setOpenNoti(false)
                }
        })
    ...
    return <div className="header-component">
        ...
        <Link href={'/'}>
            <div  className="bottion-icon">          <img src={icon.home.src}></img>Home</div>
            </Link>
